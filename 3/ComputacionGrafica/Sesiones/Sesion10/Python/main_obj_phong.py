...
from modelo_phong import Modelo  # Maneja la carga y renderizado de modelos en formato .obj
...
from luces import configurar_luces  # Configuración de la iluminación

# Inicialización de la cámara y del modelo 3D
camara = Camara()  # Crea una instancia de la cámara
modelo = Modelo("modelos/esfera.obj", draw_type=GL_TRIANGLES)  # Carga el modelo 3D en formato .obj con un estilo de línea
...
def inicializar_escena():
    ...
    # Crea la ventana con doble buffer y compatibilidad con OpenGL
    ...
    configurar_luces()  # Activa la configuración de luces después de inicializar OpenGL
...
def renderizar():
    ...
    glDisable(GL_LIGHTING) # Desactiva los efectos de iluminación para dibujar los elementos auxiliares
    dibujar_elementos_auxiliares(ejes=True, rejilla=True)  # Dibuja ejes y rejilla
    glEnable(GL_LIGHTING)  # Reactiva los efectos de iluminación
    modelo.dibujar(t_x=0, t_y=0, t_z=0, angulo=0, eje_x=0, eje_y=1, eje_z=0, sx=1, sy=1, sz=1)  # Dibuja el modelo en el origen
