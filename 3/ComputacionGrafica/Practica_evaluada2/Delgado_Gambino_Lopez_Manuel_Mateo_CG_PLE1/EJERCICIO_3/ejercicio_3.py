...
def render_points():
    glBegin(GL_POINTS)
    for point in points:
        glColor3f(*point[1])  # Aplica el color del vértice
...
def render_lines():
    ...
    for line in lines:
        if len(line) == 2:
            glColor3f(*line[0][1])  # Aplica el color del primer vértice
            glVertex2f(line[0][0][0], line[0][0][1])
            glColor3f(*line[1][1])  # Aplica el color del segundo vértice
            glVertex2f(line[1][0][0], line[1][0][1])
    glEnd()
...
def render_triangles():
    ...
    for triangle in triangles:
        if len(triangle) == 3:
            glColor3f(*triangle[0][1])  # Aplica el color del primer vértice
            glVertex2f(triangle[0][0][0], triangle[0][0][1])
            glColor3f(*triangle[1][1])  # Aplica el color del segundo vértice
            glVertex2f(triangle[1][0][0], triangle[1][0][1])
            glColor3f(*triangle[2][1])  # Aplica el color del tercer vértice
            glVertex2f(triangle[2][0][0], triangle[2][0][1])
    glEnd()
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            vertex = ((mouse_x, mouse_y), current_color)
