...
def Entrada_CSV(file):
    import csv
    with open(file) as f:
        a = [[float(x) for x in row] for row in csv.reader(f)]
    f.close()
    return a
...
def Entrada_TXT(file):
    f = open(file)
    fila = f.read()
    f.close()
    a = [[float(i) for i in k.split(",")] for k in fila.split("\n")]
    return a
